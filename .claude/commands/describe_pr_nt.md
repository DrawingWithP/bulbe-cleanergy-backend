...
1. **Leia o template de descrição de PR:**
    ...
    - Use o seguinte template de descrição de PR:
...
3. **Verifique se já existe uma descrição:**
   - Verifique se o `/tmp/{repo_name}/prs/{number}_description.md` já existe
...
8. **Salve e sincronize a descrição:**
   - Escreva a descrição finalizada em `/tmp/{repo_name}/prs/{number}_description.md`
   - Mostre a descrição gerada ao usuário

9. **Atualize o PR:**
   - Atualize a descrição do PR diretamente: `gh pr edit {number} --body-file /tmp/{repo_name}/prs/{number}_description.md`
